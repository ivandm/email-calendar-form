...
class modSimpleEmailFormCalendar {
	...
	protected $_maxFields	= 10;		// mixed From, Subject, text, textarea, dropdown select, radio, checkbox fields
	...
	public function __construct($params) 
	{
		...
		$this->_inputClassDate  = $this->_cssClass . "_input_date";
		...
		// Load language files
		// i.e. tr-TR.mod_simpleemailformcalendar.ini
		$this->_lang = $params->get('mod_simpleemailform_defaultLang');
		$langFile = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'language_files' 
				  . DIRECTORY_SEPARATOR . $this->_lang . '.mod_simpleemailformcalendar.ini';
		if (file_exists($langFile)) {
			$this->_transLang = parse_ini_file($langFile);
		} else {
			$langFile = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'language_files' 
					  . DIRECTORY_SEPARATOR . 'en-GB.mod_simpleemailformcalendar.ini';
			$this->_transLang = parse_ini_file($langFile);
		}
			...
			if (stripos('-YR', $this->_field[$x]['active'])) {
                ...
                switch ($this->_field[$x]['from']) {
					...
					case 'F' :
						...
						break;
                    case 'Y' :
						// 2014-11-28 IVAN: added HTML5 date type field if email checking is enabled
						// from field
						$type = 'text';
						$row .= sprintf('<input type="%s" name="%s" id="%s" size="%d" value="%s" maxlength="%d" class="%s"/>',
										$type, 
										...
										$value, 
										$this->_field[$x]['maxx'], 
										'' //$this->_inputClassDate
                                        );
                        $this->dataRangePicker($name);
						break;
					default :
    ...
    // DateRangePicker
    public function dataRangePicker($name=''){
        // Add style sheet and css 
        // Load css and javascript
        $cssFile = '/modules/mod_simpleemailformcalendar/css/daterangepicker.css';
        $jsFileMoment = '/modules/mod_simpleemailformcalendar/js/moment.min.js';
        $jsFileDatePicker = '/modules/mod_simpleemailformcalendar/js/jquery.daterangepicker.js';
       
        $doc = JFactory::getDocument();
        $doc->addStyleSheet($cssFile);
        //$doc->addStyleSheet('/modules/mod_simpleemailformcalendar/css/bootstrap.min.css');
        //$doc->addStyleSheet('/modules/mod_simpleemailformcalendar/css/font-awesome.min.css');
        $doc->addScript($jsFileMoment);
        $doc->addScript($jsFileDatePicker);
        // Add Javascript inline
        $doc->addScriptDeclaration('
            jQuery(document).ready(function() {
                jQuery("input' . "[name='" . $name . "']" . '").dateRangePicker(
                    { 
                        format: "DD/MM/YY",
                        startDate: moment(),
                        minDate: moment(),
	...
	public function sendResults(_SimpleEmailForm &$msg, $field)
	{
		...
		// 2013-11-07 DB: added option to populate "From" field with predefined value if email check is disabled
		if (!$this->_emailCheck) {
			$msg->from = $this->_field[$this->_fromField]['value'];
		}
